...
- **Project document** — the file the host composes from every instruction source (for Claude, `CLAUDE.md`), where it is mounted, and whether its shipped base file is protected from operator mounts.
...
- **Skill backings and views** — where the provider's native skills directory lives, whether core keeps shared-skill symlinks in it, and any extra bind mounts of it into the container.
- **Prepared files** — files core creates or reconciles inside a state volume (for Claude, `settings.json`).
