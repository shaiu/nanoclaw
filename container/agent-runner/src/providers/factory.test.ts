...
function minimalContract(): ProviderRuntimeContract {
  return {
    ...
    configuration: { executionPolicy: { value: { boundary: 'container' } } },
    textDelivery: 'result',
  ...
  it('resolves the declared configuration itself and hands it to the provider factory', () => {
    ...
    registerProvider(name, {
      ...
      contract: {
        seamVersion: PROVIDER_RUNTIME_CONTRACT_SEAM_VERSION,
        configuration: {
          executionPolicy: { resolve: (_input, environment) => ({ home: environment.HOME ?? null }) },
          inference: { resolve: (input) => ({ chosen: input.model ?? 'default' }) },
          memory: { resolve: (hook) => ({ hookCommand: hook.command }) },
          mcpServers: { value: { fixed: true } },
        },
        textDelivery: 'result',
        commands: { formatting: 'xml' },
      },
